		...
		switch ($action) {
			...
			case 'POST_exercices' : 
				...
				if ($nom = valider("nom"))
				{
					...
					$id = mkExercice($nom, $unite, $alcoolise);
...
/* INUTILE ? on ne doit pas pouvoir créer de cocktails pour d'autres
			case 'POST_users_cocktails' :
				// POST /api/users/<id>/cocktails?nom=...
				if ($idEntite1)
				if (($titre = valider("nom")) !== false) {
					if ($connectedId != $idEntite1) {
						$data["status"] = 403;
					} else {
						$id = mkCocktail($idEntite1, $titre); 
						$data["cocktail"] = getCocktail($id);
						$data["success"] = true;
						$data["status"] = 201;
					}
				}
...
/*	INUTILE ? 
			case 'PUT_users_cocktails' : 
				// PUT /api/users/<id>/cocktails/<id>?nom=...
				if ($idEntite1)
				if ($idEntite2)
				if (($titre = valider("nom")) !== false) {
					if ($connectedId != $idEntite1) {
						...
						if (chgNomCocktail($idEntite2,$titre,$idEntite1)) {
							$data["article"] = getCocktail($idEntite2);
							$data["success"] = true; 
							$data["status"] = 200;
						} else {
							// erreur
...
*/			
			...
			case 'PUT_exercices' : 
				...
				if ($idEntite1)
				if ( (($unite = valider("unite")) !== false) || isset($_REQUEST["unite"]) ){
					// Les unités doivent pouvoir être vides ! 
					if (chgUniteExercice($idEntite1,$unite)) {
							$data["exercice"] = getExercice($idEntite1);
							$data["success"] = true; 
							$data["status"] = 200;
						} else {
			...
			case 'PUT_cycles' : 
				...
				if ($idEntite1) 
				if ( (($commentaires = valider("commentaires")) !== false) || (isset($_REQUEST["commentaires"])) ){
					// en rajountant le OU, on profite de la création du commentaire false !!
					if (!isUserOwnerOfCocktail($connectedId,$idEntite1)) {
						$data["status"] = 403;
					} else {						
						if (chgCommentairesCycle($idEntite1,$commentaires,$connectedId)) {
							$data["cycle"] = getCycle($idEntite1);
							$data["success"] = true; 
							$data["status"] = 200;
						} else {
			...
			case 'PUT_cycles_exos' : 
				// NB : on ne doit pas pouvoir changer d'id d'ingrédient, juste la quantité 
				// la quantité ne peut être nulle 
				// TODO : il faut vérifier si le composant existe encore ! 
				// PUT /api/cocktails/<id>/composants/<id>?quantite=...
				if ($idEntite1)
				if ($idEntite2)
				if (($quantite = valider("quantite")) !== false) {
					if (!isUserOwnerOfCycle($connectedId,$idEntite1)) {
						$data["status"] = 403;
					} else {
						if (chgQuantiteExo($idEntite2,$quantite,$idEntite1)) {
							$data["exo"] = getExo($idEntite2,$idEntite1);
							$data["success"] = true; 
							$data["status"] = 200;
						} else {
							// erreur
